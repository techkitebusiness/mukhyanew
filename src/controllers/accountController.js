...
const register = async (req, res) => {
    ...
    let time = timeCreate();
    let joiningBonusStr = process.env.JOINING_BONUS; // Read environment variable as string
    // Convert the string value to an integer
    let joiningBonus = parseInt(joiningBonusStr);
        ...
        if (check_u.length == 1 && check_u[0].veri == 1) {
            ...
            if (check_i.length == 1) {
                if (check_ip.length <= 3) {
                    ...
                    await connection.execute(sql, [id_user, username, name_user, md5(pwd), pwd, joiningBonus, code, invitecode, ctv, 1, otp2, ip, 1, time]);
